...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['create', 'list', 'retrieve', 'approve_quotation', 'upload_payment', 'cancel_order']:
    ...
    def perform_create(self, serializer):
        try:
            # Matches the related_name='customer_profile' from your unified models.py
            customer = self.request.user.customer_profile
            serializer.save(
                user=self.request.user,
                company_name=customer.company_name,
                company_address=getattr(customer, 'company_address', ''),
    ...
    def send_quotation(self, request, pk=None):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                new_revision = (existing_quotation.revision_number + 1) if existing_quotation else 0
    ...
    def submit_inspection(self, request, pk=None):
        order = self.get_object()
        result = request.data.get('result')     # 'Approved' | 'Rejected' | 'Re-inspection'
        remarks = request.data.get('remarks')   # Technical logs
        new_date = request.data.get('new_date') # Next.js client component state payload
        ...
        with transaction.atomic():
            ...
            # 🟢 SAFELY INTERCEPTS LOGS INTO YOUR Standalone SiteInspection Schema row
            ...
            if result == 'Approved':
                # State Machine Routing logic based on agreed payment structure
                ...
                if final_schedule_date:
                    Schedule.objects.update_or_create(
                        order=order,
                        defaults={'delivery_date': final_schedule_date}
                    )
        ...
        return Response({
            "message": f"Site {result} successfully completed.",
    ...
    def upload_payment(self, request, pk=None):
        ...
        try:
            ...
            return Response({"error": f"Database integrity anomaly: {str(e)}"}, status=500)
    ...
    def cancel_order(self, request, pk=None):
        ...
        try:
            with transaction.atomic():
                ...
                # Check if the user is authenticated before saving the relationship
                if request.user and request.user.is_authenticated:
                    order.canceled_by = request.user
                else:
                    order.canceled_by = None  # Or skip assigning if it can't be null
            ...
            return Response({
                ...
                "new_status": order.status,
                # Safe fallbacks to prevent runtime AttributeError crashes
                "cancelled_by": request.user.username if request.user.is_authenticated else "System/Anonymous",
                "role": getattr(request.user, 'role', 'No Role') if request.user.is_authenticated else "Guest"
            }, status=status.HTTP_200_OK)
            
        except Exception as e:
            return Response(
                {"error": f"Failed to halt order processing pipeline: {str(e)}"}, 
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def verify_payment(self, request, pk=None):
        ...
        if decision not in ['approve', 'reject']:
            return Response({"error": "Verdict must be 'approve' or 'reject'."}, status=400)
            
        if decision == 'reject' and not remarks_input:
            return Response({"error": "A rejection reason is required when denying a payment transaction."}, status=400)
        ...
        try:
            with transaction.atomic():
                ...
                if decision == 'approve':
                    ...
                    order.status = 'Ready for Pouring' 
                    order.save()
                    message_response = "Payment verified successfully. Order advanced to logistics."
                else: 
                    ...
                    order.status = 'Quotation Sent' 
                    order.save()
                    message_response = f"Payment rejected. Reason logged: '{remarks_input}'"
                    
            return Response({"status": "success", "message": message_response}, status=200)
        except Exception as e:
            return Response({"error": f"Operational failure: {str(e)}"}, status=500)


# ==========================================
# METRICS & LOGISTICS VIEWS
# ==========================================
...
def admin_dashboard_analytics(request):
    try:
        ...
        # 🟢 FIXED: Simplified volume calculation using the strict related_name model map
        total_volume = 0
        for o in active_orders:
            total_volume += sum(item.volume for item in o.order_items.all() if item.volume)
...
class ScheduleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Schedule.objects.select_related('order', 'order__delivery').prefetch_related('order__order_items__mix_design').all()
