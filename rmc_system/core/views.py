...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        # 🟢 FIXED: Unified all safe/customer actions including payment uploads into a single conditional routing array
        if self.action in ['create', 'list', 'retrieve', 'approve_quotation', 'upload_payment']:
            return [permissions.IsAuthenticated()]
        
        # Administrative control actions (send_quotation, preview, submit_inspection) require full Admin role validation
        return [IsAdminUserRole()]
