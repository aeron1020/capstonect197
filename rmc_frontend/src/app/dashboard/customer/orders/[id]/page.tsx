...
import api from '@/src/lib/api';
import QuotationView from '@/src/components/QuotationView';
import { CheckCircle, AlertCircle, ArrowLeft, XCircle, Upload, FileCheck, Loader2, ClipboardCheck, FileText, Wallet, CalendarCheck, Ban, PhoneCall } from 'lucide-react';
import Link from 'next/link';
import Swal from 'sweetalert2';
...
export default function OrderDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Dynamic Payment Term flags based on backend values (e.g., 'COD', 'Cash on Delivery', 'Advance')
  const paymentTermsRaw = order?.payment_terms || order?.payment_term || "";
  const paymentTermsNormalized = String(paymentTermsRaw).toUpperCase();
  const paymentMethodNormalized = order?.payment_method?.toUpperCase() || "";
  const hasPaymentProof = order?.payments?.length > 0;
  const isCOD = 
    paymentTermsNormalized.includes('COD') || 
    paymentTermsNormalized.includes('CASH ON DELIVERY') || 
    paymentMethodNormalized.includes('COD') || 
    paymentMethodNormalized.includes('CASH ON DELIVERY');
  const isCODPaymentVerificationPending = isCOD && order.status === 'Delivered' && order.payment_status !== 'Paid' && !hasPaymentProof;
  const isCODBypassed = isCOD && !isCODPaymentVerificationPending;
  const showPaymentUpload = !isInspectionRejected && ((order.status === 'For Payment Verification' && !isCOD) || isCODPaymentVerificationPending);
        ...
        <div className="lg:col-span-1 space-y-6 print:hidden">
          ...
          <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
            <div>
              ...
              <div className="space-y-4">
                ...
                {/* SYSTEM NOTICE: EARLY SCHEDULE CANCELLATION WARNING */}
                <div className="p-3 bg-rose-50/80 border border-rose-100 rounded-2xl space-y-1.5">
                  <div className="flex items-center gap-1.5 text-rose-700">
                    <PhoneCall className="w-3.5 h-3.5 shrink-0" />
                    <p className="text-[9px] font-black uppercase tracking-wider">Cancellation Policy</p>
                  </div>
                  <p className="text-[10px] text-rose-600/90 leading-normal font-medium">
                    To modify or cancel your scheduled pouring slot, please contact logistics immediately at:
                  </p>
                  <span className="block font-mono font-black text-xs text-rose-900 tracking-wide bg-rose-100/60 px-2 py-1 rounded-lg text-center border border-rose-200/50">
                    0917 127 6900
                  </span>
                </div>
              </div>
          ...
          <div>
            ...
            <div className="space-y-5 relative before:absolute before:bottom-2 before:top-2 before:left-[11px] before:w-[2px] before:bg-gray-100">
              ...
              {/* NEW STEP 3: PAYMENT VERIFICATION (HANDLES COD BYPASS) */}
              <div className="flex gap-3 relative items-start">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 z-10 text-[10px] font-bold ${
                  isCODPaymentVerificationPending ? 'bg-amber-500 text-white ring-4 ring-amber-100' :
                  isCOD ? 'bg-slate-200 text-slate-500 ring-2 ring-slate-100' :
                  getStepStatus('For Payment Verification') === 'completed' ? 'bg-emerald-500 text-white' :
                  getStepStatus('For Payment Verification') === 'active' ? 'bg-amber-500 text-white ring-4 ring-amber-100' : 'bg-gray-100 text-gray-400'
                }`}>
                  <Wallet className="w-3 h-3" />
                ...
                <div className="flex flex-col flex-1">
                  <div className="flex items-center justify-between gap-1">
                    <p className={`text-[11px] font-bold uppercase tracking-tight ${
                      isCODPaymentVerificationPending ? 'text-amber-600' :
                      isCOD ? 'text-slate-400 font-medium' :
                      getStepStatus('For Payment Verification') === 'active' ? 'text-amber-600' : 'text-gray-700'
                    } ${isInspectionRejected ? 'line-through text-gray-300' : ''}`}>
                      3. Payment Verification
                    </p>
                    {isCODPaymentVerificationPending ? (
                      <span className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded-md tracking-wide shrink-0 bg-amber-50 text-amber-700 border border-amber-100">
                        COD Payment Pending
                      </span>
                    ) : isCOD ? (
                      <span className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded-md tracking-wide shrink-0 bg-slate-100 text-slate-500 border border-slate-200">
                        COD Bypassed
                      </span>
                    ) : null}
                  </div>
                  <p className="text-[10px] text-gray-400 leading-tight mt-0.5">
                    {isCOD 
                      ? (isCODPaymentVerificationPending
                          ? "COD order delivered. Upload payment proof here so accounting can verify your transaction." 
                          : "Terms set to Cash on Delivery. Advance payment verification skipped for immediate pouring clearance.")
                      : "Advance remittance validation required via receipt attachments."}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3 relative items-start">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 z-10 text-[10px] font-bold ${
                  order.status === 'Ready for Pouring' || order.status === 'Delivered' || (isCOD && inspection?.result === 'Approved' && order.status !== 'Pending' && order.status !== 'Quotation Sent')
          ...
          {showPaymentUpload && (
            <div className="bg-white p-6 rounded-3xl shadow-md border-2 border-amber-200/70 space-y-4">
              ...
              <p className="text-gray-500 text-[11px] leading-relaxed">
                {isCODPaymentVerificationPending
                  ? 'Your COD order has been delivered. Upload proof of payment now so accounting can verify and close this transaction.'
                  : 'Please attach your bank transfer receipt, deposit slip, or electronic transaction receipt screenshot to secure your pouring slot clearance.'}
              </p>
